#!/usr/bin/env node
// Bumps the three SDK package versions in one shot. Adapted from the
// tiny.place SDK release tooling.
//
//   node scripts/bump-versions.mjs <patch|minor|major> [--only <list>] [--sync] [--dry-run]
//
// Targets: typescript (sdk/typescript/package.json),
//          python (sdk/python/pyproject.toml),
//          rust (sdk/rust/Cargo.toml + Cargo.lock).
...
const PACKAGES = [
  ...
  },
  {
    ...
    writeVersion: (path, version) => {
      ...
      writeCargoLockVersion(resolvePath("sdk/rust/Cargo.lock"), "tinyhumans-sdk", version);
    },
  },
];
...
function usage(exitCode = 0) {
  ...
  stream.write("Usage: node scripts/bump-versions.mjs <patch|minor|major> [--only <list>] [--sync] [--dry-run]\n\n");
  ...
  stream.write("By default all three SDKs are bumped, each from its own current version.\n");
  stream.write("With --only typescript,rust (comma/space separated), only the named targets are bumped.\n");
  stream.write("With --sync, the selected targets are set to the same next version.\n");
...
function main() {
  ...
  const positional = args.filter((arg, index) => {
    if (onlyIndex !== -1 && (index === onlyIndex || index === onlyIndex + 1)) return false;
  ...
  if (!["patch", "minor", "major"].includes(bump) || unknown.length > 0) usage(1);
  ...
  const current = selectedPackages.map((pkg) => {
    ...
    return { ...pkg, absolutePath, currentVersion: pkg.readVersion(absolutePath) };
  });
  ...
  for (const pkg of current) {
    ...
    if (!dryRun) pkg.writeVersion(pkg.absolutePath, version);
    process.stdout.write(`${pkg.name}: ${pkg.currentVersion} -> ${version}\n`);
  }

  if (dryRun) process.stdout.write("dry run: no files changed\n");
}
...
function parseVersion(version) {
  ...
  if (!match) throw new Error(`Unsupported version "${version}". Expected MAJOR.MINOR.PATCH.`);
...
function bumpVersion(version, bump) {
  ...
  if (bump === "major") return formatVersion({ major: parsed.major + 1, minor: 0, patch: 0 });
  if (bump === "minor") return formatVersion({ major: parsed.major, minor: parsed.minor + 1, patch: 0 });
...
function writePackageJsonVersion(path, version) {
  ...
  if (!replaced) throw new Error(`Missing top-level "version" field in ${path}`);
...
function readTomlVersion(path, sectionName) {
  ...
  if (!section) throw new Error(`Missing [${sectionName}] section in ${path}`);
  const version = section[2].match(/^version\s*=\s*"([^"]+)"\s*$/m);
  if (!version) throw new Error(`Missing version field in [${sectionName}] section of ${path}`);
  return version[1];
}
...
function writeTomlVersion(path, sectionName, version) {
  ...
  if (updated === text) throw new Error(`Failed to update [${sectionName}] version in ${path}`);
  writeFileSync(path, updated);
}
...
function writeCargoLockVersion(path, packageName, version) {
  ...
  const updated = text.replace(packagePattern, (match, header, body) => {
    if (!new RegExp(`^name\\s*=\\s*"${escapeRegExp(packageName)}"\\s*$`, "m").test(body)) return match;
    found = true;
    return `${header}${body.replace(/^version\s*=\s*"[^"]+"\s*$/m, `version = "${version}"`)}`;
  });
  if (!found) throw new Error(`Missing ${packageName} package entry in ${path}`);
  writeFileSync(path, updated);
}
